...
- **Case risk modal** is now sized to the screen, like every other modal in the app. The hero (case number, status, recommendation), the tabs, and the footer all stay pinned — the only thing that scrolls is the tab content itself. Long enumeration lists inside (like the 108-feature table) keep their own bounded scroll boxes so they don't push the rest of the tab content out of view.
...
- `src/utils/caseRiskPredictions.js` (`CaseRiskAnalyticsModal`): outer wrapper switched from `items-start … overflow-y-auto p-6` to `items-center … p-4`; container now `max-h-[90vh] flex flex-col overflow-hidden`. Inside the container the four sections are direct flex children: hero `flex-shrink-0`, tab strip `flex-shrink-0`, body `flex-1 overflow-y-auto min-h-0`, footer `flex-shrink-0`. The body is the only scrolling element. `FeatureTable` keeps its existing `max-h-[460px] overflow-y-auto` so the 108-row feature list scrolls inside its own box without pushing the intro card or filter buttons offscreen.
